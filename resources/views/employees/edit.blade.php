                    ...
                    <form method="POST" action="{{ route('employees.update', $employee) }}" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input name="employee_code" value="{{ old('employee_code', $employee->employee_code) }}" class="mt-1 w-full rounded border-gray-300" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Name</label>
                                <input name="name" value="{{ old('name', $employee->name) }}" required class="mt-1 w-full rounded border-gray-300" />
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input name="email" value="{{ old('email', $employee->email) }}" class="mt-1 w-full rounded border-gray-300" />
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input name="designation" value="{{ old('designation', $employee->designation) }}" class="mt-1 w-full rounded border-gray-300" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Joining Date</label>
                                <input type="date" name="joining_date" value="{{ old('joining_date', optional($employee->joining_date)->toDateString()) }}" class="mt-1 w-full rounded border-gray-300" />
                            </div>
                            <div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                                @php $current = $employee->roles->pluck('id')->toArray(); @endphp
                                @foreach($roles as $role)
                                    <label class="flex items-center gap-2">
                                        <input type="checkbox" name="role_ids[]" value="{{ $role->id }}" class="rounded border-gray-300"
                                               @checked(in_array($role->id, old('role_ids', $current))) />
