                    ...
                    <form method="POST" action="{{ route('suppliers.update', $supplier) }}" class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input name="phone" value="{{ old('phone', $supplier->phone) }}" class="mt-1 w-full rounded border-gray-300" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Email</label>
                                <input name="email" value="{{ old('email', $supplier->email) }}" class="mt-1 w-full rounded border-gray-300" />
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Address</label>
                            <textarea name="address" rows="3" class="mt-1 w-full rounded border-gray-300">{{ old('address', $supplier->address) }}</textarea>
                        </div>
