                ...
                <div class="p-6">
                    ...
                    <form method="POST" action="{{ route('units.update', $unit) }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input name="name" value="{{ old('name', $unit->name) }}" class="mt-1 w-full rounded border-gray-300" required>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Symbol</label>
                            <input name="symbol" value="{{ old('symbol', $unit->symbol) }}" class="mt-1 w-full rounded border-gray-300" required>
                        </div>

                        <div class="flex items-center gap-2">
                            <input id="allow_decimal" name="allow_decimal" type="checkbox" value="1" class="rounded border-gray-300"
                                   {{ old('allow_decimal', $unit->allow_decimal) ? 'checked' : '' }}>
